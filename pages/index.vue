...
const onSubmit = async () => {
    ...
    if (!error.value) {
        ...
        if (userData?.data?.value) {
            ...
            else if(userData.data.value.type === authType.ORGANIZER){
                defaultNavigate = "/organizer/dashboard";
            }
            else{
                defaultNavigate = "/user/project";
            }
            return navigateTo(defaultNavigate)
        }
    }
    pending.value = false;
}
                        ...
                        <form class="space-y-5 dark:text-white" @submit.prevent="onSubmit">
                            <div>
                                ...
                                <div class="relative text-white-dark">
                                    <input id="Email" type="email" v-model="state.email" placeholder="Enter Email"
                                        :disabled="pending"
                                ...
                                <p v-if="errors?.errors?.email" class="text-red-500">{{ errors?.errors?.email[0] }}</p>
                            </div>
                            <div>
                                ...
                                <p v-if="errors?.errors?.password" class="text-red-500">{{ errors?.errors?.password[0] }}</p>
                            </div>
                            <button type="submit" :disabled="pending" class="btn btn-info !mt-6 w-full border-0 uppercase shadow-[0_10px_20px_-10px_rgba(67,97,238,0.44)]">
                                <span v-if="pending"  class="inline-flex h-5 w-5 animate-spin rounded-full border-2 border-white !border-l-transparent dark:border-black"></span>
